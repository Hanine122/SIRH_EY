...
public class RhInsightsController : Controller
{
    ...
    private void BuildExecutiveKpis(RhInsightsViewModel model, List<Collaborateur> tousActifs, List<Formation> formations)
    {
        ...
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Préparation à la succession", Value = $"{successionReadiness:0}%", NumericValue = successionReadiness, ValueSuffix = "%", Trend = "+5%", IconClass = "fas fa-user-shield", ColorClass = "text-success", Tone = "success", Subtitle = "Couverture des relais possibles", Insight = "Relais prioritaires detectes sur les roles critiques." });
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Risque de continuité", Value = $"{continuityRisk:0}%", NumericValue = continuityRisk, ValueSuffix = "%", Trend = continuityRisk > 30 ? "+3%" : "-2%", IconClass = "fas fa-exclamation-triangle", ColorClass = "text-warning", Tone = continuityRisk > 30 ? "warning" : "success", Subtitle = "Risque sur postes vacants ou passation", Insight = $"{continuityAlerts} alerte(s) de continuite active(s)." });
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Positions critiques", Value = criticalPositions.ToString(), NumericValue = criticalPositions, Trend = "+1", IconClass = "fas fa-sitemap", ColorClass = "text-danger", Tone = "danger", Subtitle = "Roles sensibles a monitorer", Insight = "Managers et postes de continuite agreges." });
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Talents à haut potentiel", Value = highPotentialCount.ToString(), NumericValue = highPotentialCount, Trend = "+4", IconClass = "fas fa-star", ColorClass = "text-primary", Tone = "primary", Subtitle = "Collaborateurs a fort signal", Insight = $"{Percent(highPotentialCount, activeCount):0}% du vivier actif." });
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Impact de la formation", Value = $"{formationImpact:0}%", NumericValue = formationImpact, ValueSuffix = "%", Trend = "+7%", IconClass = "fas fa-graduation-cap", ColorClass = "text-info", Tone = "info", Subtitle = "Progression formation exploitable", Insight = $"{completedTrainings} formation(s) terminee(s) ou avancee(s)." });
        model.KpiCards.Add(new ExecutiveKpiCardViewModel { Title = "Indice de pénurie de compétences", Value = $"{shortageIndex:0}%", NumericValue = shortageIndex, ValueSuffix = "%", Trend = shortageIndex > 35 ? "+2%" : "-1%", IconClass = "fas fa-layer-group", ColorClass = "text-secondary", Tone = shortageIndex > 35 ? "warning" : "neutral", Subtitle = "Tension sur competences rares", Insight = "Index base sur ecarts niveau actuel / cible." });
    }
