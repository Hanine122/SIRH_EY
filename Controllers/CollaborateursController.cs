...
[AllowAnonymous]
[HttpGet("formations-recommandees/{collaborateurId}")]
public async Task<IActionResult> GetFormationsRecommandees(int collaborateurId)
{
    ...
    var competencesEnEcart = (collaborateur.Competences ?? Enumerable.Empty<Competence>())
        .Where(c => c.NiveauActuel < c.NiveauCible)
        .ToList();

    var formations = await _context.Formations
        .Where(f => f.CompetenceVisee != null)
        .AsNoTracking()
        .ToListAsync();

    var recommandations = competencesEnEcart.Select(c => {
        var f = formations.FirstOrDefault(x =>
            x.CompetenceVisee != null &&
            x.CompetenceVisee.Equals(c.Nom, StringComparison.OrdinalIgnoreCase));
        return new {
            competence     = c.Nom,
            niveauActuel   = c.NiveauActuel,
            niveauCible    = c.NiveauCible,
            ecart          = c.NiveauCible - c.NiveauActuel,
            formation      = f?.Titre ?? $"Parcours recommandé — {c.Nom}",
            plateforme     = f?.Plateforme ?? "EY Learning",
            dureeEstimee   = $"{(c.NiveauCible - c.NiveauActuel) * 4} semaines"
        };
    }).ToList();
    ...
    return Ok(new {
        ...
        totalEcarts      = recommandations.Count,
        recommandations
    });
}

[AllowAnonymous]
[HttpGet("promotion-readiness/{collaborateurId}")]
public async Task<IActionResult> GetPromotionReadiness(int collaborateurId)
{
    var collaborateur = await _context.Collaborateurs
        .Include(c => c.Competences)
        .FirstOrDefaultAsync(c => c.Id == collaborateurId);
    ...
    var result = await _promotionReadinessService
        .EvaluerAsync(collaborateurId);

    return Ok(new {
        collaborateurNom  = $"{collaborateur.Prenom} {collaborateur.Nom}",
        gradeActuel       = collaborateur.Grade,
        readinessScore    = result?.Score ?? 0,
        readinessLabel    = result?.Label ?? "Non évalué",
        competencesOk     = result?.CompetencesOk ?? new List<string>(),
        competencesManquantes = result?.CompetencesManquantes ?? new List<string>(),
        delaiEstime       = result?.DelaiEstime ?? "Non calculé",
        recommandations   = result?.Recommandations ?? new List<string>()
    });
}

[AllowAnonymous]
[HttpGet("rh-insights")]
public async Task<IActionResult> GetRhInsights()
{
    ...
    var totalActifs     = collaborateurs.Count;
    var tauxFormation   = inscriptions.Any()
        ? Math.Round(inscriptions.Count(i => i.Terminee) * 100.0 / inscriptions.Count, 1)
        : 0;

    var parDept = collaborateurs
        .GroupBy(c => c.Departement ?? "Non défini")
        .Select(g => new { departement = g.Key, total = g.Count() })
        .OrderByDescending(x => x.total)
        .ToList();

    var scoresMoyens = collaborateurs
        .Where(c => c.Competences.Any())
        .Select(c => c.Competences.Average(x => (double)x.NiveauActuel))
        .ToList();

    var scoreMoyenGlobal = scoresMoyens.Any()
        ? Math.Round(scoresMoyens.Average(), 1)
        : 0;
    ...
    return Ok(new {
        ...
        tauxFormation,
        scoreMoyenGlobal,
        repartitionDepartements = parDept,
        totalFormations         = inscriptions.Count,
        formationsTerminees     = inscriptions.Count(i => i.Terminee),
        formationsEnCours       = inscriptions.Count(i => !i.Terminee)
    });
}
